...
module.exports = async function fileNoteRoutes(fastify) {
  ...
  fastify.route({
    ...
    schema: {
      body: {
        ...
        description:
          "Import a note list from a CSV file with the following format: title,done",
        properties: {
          noteListFile: {
            type: "array",
            items: {
    ...
    handler: async function listNotes(request, reply) {
      const inserted = await request.notesDataSource.createNotes(
        request.body.noteListFile,
      );
      reply.code(201);
      return inserted;
    },
  ...
  fastify.route({
    ...
    handler: async function listNotes(request, reply) {
      ...
      reply.header(
        "Content-Disposition",
        'attachment; filename="note-list.csv"',
      );
      reply.type("text/csv");
      return cursor.pipe(
        csvStringify({
