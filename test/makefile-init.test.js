...
test('make init-env creates local defaults once and does not overwrite existing env files', (t) => {
  ...
  assert.ok(
    secondRun.output.includes(`${envFile} already exists; leaving it unchanged.`),
    secondRun.output,
  )
...
test('make migrate loads runtime values from the selected env file', (t) => {
  ...
  assert.ok(
    result.output.includes('MONGO_URL:mongodb://localhost:27017/makefile-test'),
    result.output,
  )
...
test('make help lists canonical targets', () => {
  ...
  assert.strictEqual(result.status, 0, result.output)
  assert.ok(result.output.includes('dev'), result.output)
  assert.ok(result.output.includes('test-ci'), result.output)
  assert.ok(result.output.includes('runtime-status-json'), result.output)
  assert.ok(!ansiPattern.test(result.output), result.output)
})

test('make runtime-status-json forwards the constrained JSON status args', (t) => {
  const tempDir = createTempDir(t)
  const binDir = path.join(tempDir, 'bin')
  const npmShim = path.join(binDir, 'npm')

  fs.mkdirSync(binDir)
  fs.writeFileSync(npmShim, ['#!/bin/sh', 'printf "ARGV:%s\\n" "$*"'].join('\n'), { mode: 0o755 })

  const result = runMake(['runtime-status-json'], {
    env: {
      PATH: `${binDir}${path.delimiter}${process.env.PATH}`,
    },
  })
  ...
  assert.strictEqual(result.status, 0, result.output)
  assert.ok(result.output.includes('ARGV:run runtime:status -- --json'), result.output)
})
